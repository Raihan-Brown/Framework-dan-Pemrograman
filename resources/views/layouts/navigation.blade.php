        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    {{-- Ganti rute dashboard ke root '/' jika belum login untuk menghindari error --}}
                    <a href="{{ Auth::check() ? route('dashboard') : url('/') }}">
                        <x-application-logo class="block h-9 w-auto fill-current text-gray-800 dark:text-gray-200" />
                    </a>
                </div>

                {{-- MENU UTAMA (Hanya muncul jika sudah login) --}}
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    {{-- DROPDOWN DATA MANAGEMENT (Hanya muncul jika sudah login) --}}
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link :href="route('product-create')">
                                {{ __('Input Data Produk') }}
                            </x-dropdown-link>
                        </x-slot>
                    </x-dropdown>

                    {{-- DROPDOWN USER PROFILE (Hanya muncul jika sudah login) --}}
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
